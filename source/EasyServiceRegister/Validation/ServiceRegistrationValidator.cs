            ...
            bool validateFrameworkServices = false)
        {
            ...
            foreach (var group in serviceGroups)
            {
                ...
                issues.Add(new ValidationIssue(
                    $"Duplicate registration for service {group.Key.ServiceType.FullName}{keyInfo}. " +
            ...
            foreach (var descriptor in services.Where(s => registeredServiceTypes.Contains(s.ServiceType)))
            {
                if (descriptor.ImplementationType != null)
                {
                    ...
                    foreach (var dependency in missingDependencies)
                    {
                        ...
                        issues.Add(new ValidationIssue(
                            $"Service {descriptor.ImplementationType.FullName} depends on {dependency.FullName} " +
            ...
            foreach (var singleton in singletonServices)
            {
                if (singleton.ImplementationType != null)
                {
                    ...
                    foreach (var scopedDependency in scopedDependencies)
                    {
                        ...
                        issues.Add(new ValidationIssue(
                            $"Singleton service {singleton.ImplementationType.FullName} depends on " +
                            $"scoped service {scopedDependency.Type.FullName}{keyInfo}. This is a DI anti-pattern that can cause " +
                            $"unexpected behavior because the singleton will capture a scoped instance.",
                            ValidationSeverity.Error,
                    ...
                    foreach (var transientDependency in transientDependencies)
                    {
                        ...
                        issues.Add(new ValidationIssue(
                            $"Singleton service {singleton.ImplementationType.FullName} depends on " +
                            $"transient service {transientDependency.Type.FullName}{keyInfo}. This may cause " +
                            $"excessive object creation if the singleton frequently uses the dependency.",
                            ValidationSeverity.Warning,
                            singleton.ServiceType,
                            singleton.ImplementationType
                        ));
